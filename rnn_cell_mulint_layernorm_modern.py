  ...
  def __call__(self, inputs, state, timestep = 0, scope=None):
      ...
      with tf.variable_scope(scope or type(self).__name__, reuse=self._reuse):  # "GRUCell"
        with tf.variable_scope("Inputs"):
          inputs_concat = linear([inputs], 2 * self._num_units, False, 1.0)
        ...
        r, u = tf.split(tf.sigmoid(multiplicative_integration([inputs_concat, hidden_state_concat], 2 * self._num_units,
                                                              1.0, weights_already_calculated=True)),
        ...
        with tf.variable_scope("Candidate"):
          ...
          c = self._activation(multiplicative_integration([input_portion, reset_portion], self._num_units, 0.0, weights_already_calculated=True))
...
class BasicLSTMCell_MulInt_LayerNorm(RNNCell):
  ...
  def __call__(self, inputs, state, timestep = 0, scope=None):
      ...
      with tf.variable_scope(scope or type(self).__name__, reuse=self._reuse):  # "BasicLSTMCell"
        ...
        i, j, f, o = tf.split(multiplicative_integration([inputs_concat,h_concat], 4 * self._num_units,
                                                         0.0, weights_already_calculated=True),
                              axis=1, num_or_size_splits=4)
        ...
        with tf.variable_scope('layer_norm_hidden_state'):
          new_h = self._activation(layer_norm(new_c)) * tf.sigmoid(o)
