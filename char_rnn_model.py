...
import mrnn
#from rnn_cell_modern import *
from rnn_cell_mulint_modern import *
from rnn_cell_mulint_layernorm_modern import *
...
class CharRNN(object):
    ...
    def __init__(self, is_training, batch_size, num_unrollings, vocab_size,
        ...
        elif self.model == 'mlstm':
            cell_fn = mrnn.MultiplicativeLSTMCell
        elif self.model == 'mgru':
            cell_fn = mrnn.MultiplicativeGRUCell
        elif self.model == 'delta':
            cell_fn = Delta_RNN
        elif self.model == 'mi-gru':
            cell_fn = GRUCell_MulInt
        elif self.model == 'mi-lstm':
            cell_fn = BasicLSTMCell_MulInt
        elif self.model == 'ln-mi-gru':
            cell_fn = GRUCell_MulInt_LayerNorm
        elif self.model == 'ln-mi-lstm':
            cell_fn = BasicLSTMCell_MulInt_LayerNorm
    ...
    def run_epoch(self, session, data_size, batch_generator, is_training,
        ...
        for step in range(epoch_size):
            ...
            ops = [self.ppl, self.final_state, extra_op, self.summaries,
                   self.global_step]
            ...
            ppl, state, _, summary_str, global_step = results

            if (verbose > 0) and ((step + 1) % freq == 0):
                logging.info("%.1f%%, step:%d, perplexity: %.3f, speed: %.0f wps",
                             (step + 1) * 1.0 / epoch_size * 100, step + 1, ppl,
