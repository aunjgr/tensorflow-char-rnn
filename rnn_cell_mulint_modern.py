...
class GRUCell_MulInt(RNNCell):
  ...
  def __init__(self, num_units, activation=tf.tanh, gpu_for_layer=0, weight_initializer="uniform_unit",
               orthogonal_scale_factor=1.1, use_recurrent_dropout=False, recurrent_dropout_factor=0.90,
    ...
    self._recurrent_dropout_factor = recurrent_dropout_factor
  ...
  def __call__(self, inputs, state, timestep=0, scope=None):
    ...
    with tf.variable_scope(scope or type(self).__name__, reuse=self._reuse):  # "GRUCell"
      with tf.variable_scope("Gates"):  # Reset gate and update gate.
        # We start with bias of 1.0 to not reset and not udpate.
        concat = tf.sigmoid(multiplicative_integration([inputs, state], self._num_units * 2, 1.0))
        r, u = tf.split(concat, axis=1, num_or_size_splits=2)
      ...
      with tf.variable_scope("Candidate"): #you need a different one because you're doing a new linear
        ...
        c = self._activation(multiplicative_integration([inputs, state], self._num_units, 0.0))

        if self._use_recurrent_dropout and self._is_training:
          input_contribution = tf.nn.dropout(c, self._recurrent_dropout_factor)
        else:
...
class BasicLSTMCell_MulInt(RNNCell):
  ...
  def __init__(self, num_units, forget_bias = 1.0, activation=tf.tanh, gpu_for_layer=0,
               weight_initializer="uniform_unit", orthogonal_scale_factor=1.1,
               use_recurrent_dropout=False, recurrent_dropout_factor=0.90, is_training=True, reuse=None):
    self._num_units = num_units
  ...
  def __call__(self, inputs, state, timestep=0, scope=None):
    with tf.device("/gpu:"+str(self._gpu_for_layer)):
      ...
      with tf.variable_scope(scope or type(self).__name__, reuse=self._reuse):  # "BasicLSTMCell"
        ...
        if self._use_recurrent_dropout and self._is_training:
          input_contribution = tf.nn.dropout(self._activation(j), self._recurrent_dropout_factor)
        else:
          input_contribution = self._activation(j)

        new_c = c * tf.sigmoid(f + self._forget_bias) + tf.sigmoid(i) * input_contribution
        new_h = self._activation(new_c) * tf.sigmoid(o)

      return new_h, tf.contrib.rnn.LSTMStateTuple(new_c, new_h) #purposely reversed


class HighwayRNNCell_MulInt(RNNCell):
  """Highway RNN Network with multiplicative_integration"""

  def __init__(self, num_units, activation=tf.tanh, num_highway_layers=3, use_recurrent_dropout=False, recurrent_dropout_factor=0.90, is_training=True, use_inputs_on_each_layer=False):
    self._num_units = num_units
    self._activation = activation
    ...
    for highway_layer in xrange(self.num_highway_layers):
      with tf.variable_scope('highway_factor_'+str(highway_layer)):
        if self.use_inputs_on_each_layer or highway_layer == 0:
          highway_factor = self._activation(multiplicative_integration([inputs, current_state], self._num_units))
        else:
          highway_factor = self._activation(linear([current_state], self._num_units, True))
